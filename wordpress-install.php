<?php
/**
 * Plugin name: Lightseek WP install.php dropin
 * Plugin URI: https://github.com/amielucha/
...
function wp_install( string $blog_title, string $user_name, string $user_email, bool $public,
  ...
  /*
   * Switch to our awesome Lightseek theme:
	 */
  switch_theme('lightseek');
  
  /*
   * Populate the list of favourite plugins by default:
	 */
	update_user_meta( get_current_user_id(), 'wporg_favorites', 'amielucha' );
  ...
  if ( ! $user_id && empty( $user_password ) ) {
    $user_password = wp_generate_password( 20, false );
...
function wp_install_defaults( int $user_id ) {
  ...
  $cat_slug = sanitize_title( _x( 'News', 'News category' ) );
